...
def tfCheckout (accountName, region, debug){
    ...
    def exitCode = sh(returnStatus: true, script: "cp -avr ${env.WORKSPACE}/base/* ${env.WORKSPACE}/${environment}")
    if (exitCode != 0) {
        currentBuild.result = 'FAILED'
...
def tfApplyDestroy(accountName, region, action, tfPlanOk, debug, tfLogLevel){
    ...
    if (action == "apply" && tfPlanOk == true) {
        try {
            // Apply plan
            def execApply = """
                cd ${env.WORKSPACE}/${environment}; set +e; export TF_LOG=${tfLogLevel}; terraform apply -parallelism=1 plan.out; \
            """
